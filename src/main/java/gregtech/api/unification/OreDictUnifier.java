...
public class OreDictUnifier {
    ...
    public static Comparator<ItemAndMetadata> getSimpleItemStackComparator() {
        if (stackComparator == null) {
            if (ConfigHolder.useCustomModPriorities) {
                ...
                stackComparator = Comparator.comparing(stack -> stack.item.getRegistryName().getNamespace());
            }
        }
    ...
    public static void init() {
        for (String registeredOreName : OreDictionary.getOreNames()) {
            ...
            for (ItemStack itemStack : theseOres) {
                prepareOre(registeredOreName, itemStack);
            }
        }
    ...
    public static void onItemRegistration(OreRegisterEvent event) {
        prepareOre(event.getName(), event.getOre());
    }

    public static void prepareOre(String oreName, ItemStack oreStack) {
        ItemAndMetadata simpleItemStack = new ItemAndMetadata(oreStack);
        // Cache this registration by name
        stackOreDictName.computeIfAbsent(simpleItemStack, k -> new ObjectArraySet<>()).add(oreName);
        List<ItemStack> itemStackListForOreDictName = oreDictNameStacks.computeIfAbsent(oreName, k -> new ArrayList<>());
        addAndSort(itemStackListForOreDictName, oreStack.copy(), getItemStackComparator());
        // Try to transform registration name into OrePrefix + Material pair
        ...
        if (orePrefix != null && (material != null || orePrefix.isSelfReferencing)) {
            ...
            List<ItemAndMetadata> itemListForUnifiedEntry = stackUnificationItems.computeIfAbsent(unificationEntry, p -> new ArrayList<>());
